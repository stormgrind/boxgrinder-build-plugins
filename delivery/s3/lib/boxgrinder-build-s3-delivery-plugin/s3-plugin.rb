...
module BoxGrinder
  class S3Plugin < BasePlugin
    ...
    def after_init
      ...
      set_default_config_value('host', 'http://s3.amazonaws.com')
    ...
    def upload_image
      ...
      @exec_helper.execute("euca-upload-bundle -U #{@plugin_config['host']} -b #{ami_bucket_key(@appliance_config.name, @plugin_config['path'])} -m #{@ami_manifest} -a #{@plugin_config['access_key']} -s #{@plugin_config['secret_access_key']}")
    end
